...
C2C_D = 64
C2C_DROPOUT = 0.1
TRAIN_C2C = True   # Faz-2: True; ablation için False yapabilirsin
...
@torch.no_grad()
def extract_embeddings_with_c2c(
    reve,
    pos: torch.Tensor,
    c2c: C2CAttention,
    X: np.ndarray,
    ...
    Steps:
      - move batch to torch
      - apply c2c (trainable) ONLY if it is in eval/train mode handled outside
      - feed to frozen REVE
    Returns (n_win, d) numpy
    """
    embs = []
    n = X.shape[0]
    for i in range(0, n, batch_size):
        ...
        if pb.dim() == 2:
            pb = pb.unsqueeze(0)
        if pb.size(0) == 1:
            pb = pb.expand(xb2.shape[0], -1, -1)
        elif pb.size(0) != xb2.shape[0]:
            pb = pb[:1].expand(xb2.shape[0], -1, -1)
...
def main():
    ...
    # Faz-2 run'ları ayırt etmek için tag altına "phase2" klasörü ekleyelim
    run_dir = RESULTS_ROOT / tag / ("phase2_" + run_id)
    ...
    print("=" * 78)
    print(f"[{tag}] (PHASE-2) DEVICE: {DEVICE}")
    if DEVICE == "cuda":
    ...
    print(f"[{tag}] C2C: d={C2C_D}, dropout={C2C_DROPOUT}, train={TRAIN_C2C}")
    print("=" * 78, flush=True)
    ...
    # C2C module
    c2c = C2CAttention(n_channels=n_ch, d=C2C_D, dropout=C2C_DROPOUT).to(DEVICE)
    if not TRAIN_C2C:
        for p in c2c.parameters():
    ...
    for fold_idx, (test_sid, test_pkl, y_test) in enumerate(subj_items, start=1):
        ...
        # ---------- build train subject embeddings (with C2C) ----------
        ...
        for sid, pkl, y in train_items:
            ...
            # IMPORTANT: C2C is trainable; but here we only train C2C through the final loss?
            # In this Phase-2 design, we train C2C *per fold* jointly with the probe by
            # backprop through embeddings is not possible since we detach to numpy.
            #
            # Therefore: For Phase-2 v1, we will train C2C using a simple surrogate:
            # We keep C2C frozen during embedding extraction, and only compare its effect.
            #
            # To *truly* train C2C, we need an end-to-end torch path without numpy detaches.
            # We'll do that in the next iteration once you verify Phase-2 structure.
            #
            # For now: always use c2c in eval mode during extraction.
            c2c.eval()

            w_emb = extract_embeddings_with_c2c(reve, pos, c2c, X, batch_size=BATCH_SIZE, device=DEVICE)
        ...
        # ---------- test subject embedding (with C2C) ----------
        _, X_test, _ = load_window_pkl(test_pkl)
        X_test = subsample(X_test, MAX_TEST_WINDOWS_PER_SUBJ, SEED + 1000 + fold_idx)

        c2c.eval()
        w_emb_test = extract_embeddings_with_c2c(reve, pos, c2c, X_test, batch_size=BATCH_SIZE, device=DEVICE)
    ...
    print("=" * 78)
    print(f"[{tag}] (PHASE-2) DONE -> {run_dir}")
